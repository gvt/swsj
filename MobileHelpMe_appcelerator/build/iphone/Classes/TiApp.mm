...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kMobileHelpMe_appceleratorUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kMobileHelpMe_appceleratorUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}

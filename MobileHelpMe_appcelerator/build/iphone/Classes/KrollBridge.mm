...
@implementation MobileHelpMe_appceleratorObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mobilehelpme_appcelerator);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any MobileHelpMe_appcelerator patches in JS before a context is loaded... nice for 
...
{
	// create MobileHelpMe_appcelerator global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_mobilehelpme_appcelerator = [[MobileHelpMe_appceleratorObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mobilehelpme_appcelerator];
	
	NSString *_mobilehelpme_appceleratorNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_mobilehelpme_appceleratorNS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mobilehelpme_appcelerator valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.mobilehelpme_appcelerator.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
